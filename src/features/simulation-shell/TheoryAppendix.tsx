...
export function TheoryAppendix({ limits }: TheoryAppendixProps) {
  return (
    <Box
      component="section"
      sx={{
    ...
    >
      <Stack spacing={1.5}>
        <Box>
          <Typography variant="h2">Apendice teorico</Typography>
        ...
        <Box>
          ...
          >
            {limits.map((limit) => (
              <Chip key={limit} label={limit} size="small" variant="outlined" />
            ))}
          </Stack>
        </Box>
      </Stack>
