...
- O quarto lote da Fase 3 mantem `Mecanica > Rotacao > Rolamento sem escorregamento`, `Mecanica > Gravitacao > Campo gravitacional e orbitas`, `Mecanica > Fluidos basicos > Hidrostatica e empuxo` e `Mecanica > Fluidos basicos > Continuidade e Bernoulli` como simulacoes `ready`. A malha gravitacional com dois pocos foi aprovada manualmente depois do gate de fidelidade, mantendo fixture, formulas, teoria, graficos, tabela, vetores e warnings sincronizados.
- `Mecanica > Gravitacao > Curvatura gravitacional em malha 3D` e uma segunda visualizacao executavel em `ready`, separada do tecido 2D aprovado e promovida apos o gate de fidelidade e a aprovacao visual manual. Ela reutiliza a dinamica orbital, mas representa o espaco como uma rede volumetrica de cubos interligados.
- A Lua da malha 3D pode formar um terceiro poco visual amplificado e ser desligada junto de sua guia, mas continua excluida da forca, energia e periodo da orbita kepleriana principal; o toggle nao pode alterar os samples fisicos do sistema central-planeta.
...
- `Oscilacoes e Ondas > Ondas mecanicas` mantem `Onda em corda`, `Superposicao e interferencia`, `Ondas estacionarias` e `Onda longitudinal em mola` como simulacoes `ready` apos aprovacao manual; cada uma deve manter perfil 1D, probe, vetores, graficos, tabela, formulas, teoria, regimes e warnings derivados do motor compartilhado. PixiJS permanece opcional para futuras ondas 2D densas, nao como dependencia obrigatoria dessa fatia.
- `Oscilacoes e Ondas > Som` mantem `Batimentos` e `Efeito Doppler` como simulacoes `ready` apos aprovacao manual; cada uma deve manter campo volumetrico 3D de pontinhos/marcadores de pressao, graficos, tabela, formulas, teoria, regimes e warnings derivados do motor compartilhado. Web Audio API continua opcional e nao bloqueia a prova visual.
- `Oscilacoes e Ondas > Optica` mantem `Reflexao e refracao`, `Lentes e espelhos` e `Difracao e interferencia da luz` como simulacoes `analysis` ate validacao manual; cada uma deve manter modelo optico, regimes, warnings, cena Three.js, graficos, tabela, formulas e teoria derivados do mesmo sample.
...
- Sistemas de onda longitudinal em mola devem mostrar deslocamento material paralelo ao eixo de propagacao: a mola diagonal comprime e rarefaz espiras a partir do sample, sem transformar a onda em oscilacao transversal decorativa. Cores, probes, vetores e medidas de compressao precisam ler os mesmos campos usados por graficos, tabela e formulas.
- Visualizacoes de espaco-tempo em forma de tecido devem declarar explicitamente que sao analogias de potencial. O poco central e o poco do corpo orbital podem ser amplificados visualmente, mas suas intensidades e posicoes devem vir do sample; escalas visuais nao podem alterar orbita, campo, forca ou energia.
- Visualizacoes gravitacionais volumetricas devem partir de arestas paralelas aos eixos X, Y e Z, sem diagonais internas. A intensidade, o raio do nucleo de convergencia e o alcance visual da curvatura devem crescer com a massa materializada no sample. Dentro do nucleo, varios vertices podem coincidir exatamente no centro da massa; fora dele, a transicao deve ser continua e nao pode ser apresentada como um ponto fisico finito onde a gravidade comeca. O controle de densidade pode aumentar em ate aproximadamente `10x` a quantidade total de cubos e vertices, mas deve manter o mesmo volume, divisoes equilibradas nos tres eixos e nenhuma retroacao sobre samples; amostras intermediarias continuam suavizando cada aresta sem criar celulas adicionais. O gradiente de cor deve permanecer teal na grade quase reta e caminhar por amarelo/laranja ate vermelho conforme influencia visual e desvio aumentam; o alfa deve usar a mesma leitura local, atingindo o valor escolhido perto das massas e decaindo para um piso sutil, nao nulo, longe delas. Intensidade zero deve restaurar integralmente o teal, e alfa zero deve continuar ocultando toda a grade. Um feixe de luz didatico pode iniciar sobre uma linha da grade selecionada por plano e offsets inteiros, mas deve acumular a mudanca transversal de direcao produzida pelos mesmos pocos e preservar a tangente quando a influencia cair, sem retornar a linha-base. Toggle e progresso `0..100%` pertencem somente ao renderer, e o feixe nunca deve ser apresentado como calculo quantitativo de geodesica ou lente gravitacional.
- Os toggles do corpo em orbita e do rastro orbital sao somente visuais e independentes. Ocultar o corpo nao remove a massa nem seu poco; ocultar o rastro remove tanto a referencia eliptica quanto o trecho progressivo, sem alterar a trajetoria calculada.
...
- Os atalhos globais de velocidade devem ser unicos, permanecer dentro da faixa contratual `0..1` e aparecer como `0,1x`, `0,25x`, `0,5x`, `0,75x` e `1x` em todas as simulacoes.
