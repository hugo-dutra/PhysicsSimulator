...
- `Oscilacoes e Ondas > Ondas mecanicas` mantem `Onda em corda`, `Superposicao e interferencia` e `Ondas estacionarias` como simulacoes `ready` apos aprovacao manual, e `Onda longitudinal em mola` como simulacao `analysis` ate validacao manual; cada uma deve manter perfil 1D, probe, vetores, graficos, tabela, formulas, teoria, regimes e warnings derivados do motor compartilhado. PixiJS permanece opcional para futuras ondas 2D densas, nao como dependencia obrigatoria dessa fatia.
