...
Este guia define o catalogo planejado de simulacoes de fisica basica. Ele nao muda a regra core-first: a primeira entrega funcional continua sendo o pendulo simples, com dados locais, motor proprio e sincronizacao completa entre cena, graficos, tabela, formulas e teoria. No catalogo consolidado, o pendulo fica em `Oscilacoes e Ondas > Oscilacoes > Pendulo simples`, junto de `Massa-mola vertical` e dos osciladores da Fase 4 para evitar uma subarea repetida em Mecanica. Apos teste manual do dono do projeto, o pendulo fica como `ready`. A Fase 2 implementou `Mecanica > Dinamica > Plano inclinado com atrito` como segunda simulacao executavel para validar reuso e, apos aprovacao manual, ela fica como `ready`. A Fase 3 iniciou a expansao de Mecanica implementando quatro simulacoes de Cinematica: MRU, MUV/queda livre, lancamento obliquo e MCU. Apos aprovacao manual, MRU, MUV/queda livre, lancamento obliquo e MCU ficam como `ready`. O segundo lote implementou `Maquina de Atwood`, `Forca centripeta em curva` e `Trabalho e energia em trilho`; apos aprovacao manual, Atwood, forca centripeta e `Trabalho e energia em trilho` ficam como `ready`, com `Trabalho e energia em trilho` validada no cenario de rampa em U com perda percentual. O terceiro lote implementou `Colisoes 1D e 2D`, `Equilibrio de particula`, `Torque, alavancas e centro de massa` e `Rotacao de corpo rigido`; apos aprovacao manual, as quatro ficam como `ready`. O quarto lote implementou `Rolamento sem escorregamento`, `Campo gravitacional e orbitas`, `Hidrostatica e empuxo` e `Continuidade e Bernoulli`, alem de consolidar presets didaticos por subarea mecanica; apos aprovacao manual, `Rolamento sem escorregamento`, `Campo gravitacional e orbitas`, `Hidrostatica e empuxo` e `Continuidade e Bernoulli` ficam como `ready`. A fatia visual de `Massa-mola vertical` tambem fica em `Oscilacoes e Ondas > Oscilacoes` como `ready`, ao lado do pendulo. Apos aprovacao manual, a Fase 4 mantem `Oscilador amortecido`, `Oscilador forcado e ressonancia`, `Osciladores acoplados`, `Onda em corda`, `Superposicao e interferencia`, `Ondas estacionarias`, `Batimentos` e `Efeito Doppler` como `ready`. `Onda longitudinal em mola` entra em `Oscilacoes e Ondas > Ondas mecanicas` como `analysis`, com mola diagonal, propagacao paralela ao eixo da mola e compressao/rarefacao sincronizadas. A primeira fatia de Optica coloca `Reflexao e refracao`, `Lentes e espelhos` e `Difracao e interferencia da luz` como `analysis`, aguardando teste manual antes de qualquer promocao.

Depois da inclusao da malha deformavel com poco central e poco orbital, `Campo gravitacional e orbitas` voltou para `ready` apos passar pelo gate automatizado e receber nova aprovacao visual do dono. `Curvatura gravitacional em malha 3D` entrou como uma segunda simulacao em `analysis`, preservando o tecido aprovado e oferecendo a analogia volumetrica solicitada; depois da evolucao da malha, do feixe e do terceiro poco lunar, tambem passou para `ready` com aprovacao visual manual do dono.
...
| Gravitacao | Campo gravitacional e orbitas | Lei da gravitacao, potencial especifico, velocidade orbital, orbitas circulares/elipticas e malha do espaco-tempo com dois pocos. | Fase 3 `ready`; campo central de dois corpos, malha Three.js dinamica com poco fixo e poco orbital amplificado, graficos/tabela/formulas/teoria sincronizados e aprovacao visual manual. |
| Gravitacao | Curvatura gravitacional em malha 3D | Espaco volumetrico como cubos interligados, com linhas convergindo para as massas e vertices coincidindo em seus centros. | Extensao da Fase 3 `ready` apos aprovacao visual manual; reutiliza a orbita newtoniana e adiciona `BufferGeometry` 3D com intensidade, nucleo e alcance visual dependentes da massa. |
...
- Gravitacao: orbita baixa, elipse didatica, planeta massivo, volume ortogonal, alcance amplo e massa orbital destacada.
...
- PixiJS para campos 2D densos, interferencia e frentes de onda quando Three.js nao for suficiente.
...
A subarea `Oscilacoes e Ondas > Oscilacoes` concentra todos os osciladores: pendulo simples, massa-mola vertical, oscilador amortecido, oscilador forcado com ressonancia e osciladores acoplados. A subarea `Ondas mecanicas` mantem corda viajante, superposicao/interferencia e ondas estacionarias como `ready` apos aprovacao manual, e adiciona `Onda longitudinal em mola` como `analysis`, reaproveitando shell, blocos de graficos/tabela/formulas/teoria e padrao renderer-first; PixiJS continua opcional para futuras ondas 2D densas. A subarea `Som` mantem `Batimentos` e `Efeito Doppler` como `ready` apos aprovacao manual, priorizando a leitura visual por campo volumetrico 3D de pontinhos/marcadores de pressao em Three.js antes de qualquer Web Audio API. A subarea `Optica` mantem as tres primeiras simulacoes como `analysis`, com modelos analiticos/geometricos e laboratorio Three.js compartilhado.
...
| Oscilacoes | Oscilador amortecido | Subamortecido, criticamente amortecido, superamortecido, energia dissipada e warnings de regime. | Fase 4 `ready` apos aprovacao manual; solucao analitica por regimes, Three.js e live-canvas. |
| Oscilacoes | Oscilador forcado e ressonancia | Frequencia externa, amplitude de resposta, fase, trabalho externo, dissipacao e ressonancia. | Fase 4 `ready` apos aprovacao manual; integracao RK4 deterministica, Three.js e live-canvas. |
| Oscilacoes | Osciladores acoplados | Modos normais, batimentos, troca de energia, modo comum e fase relativa. | Fase 4 `ready` apos aprovacao manual; integrador RK4 para duas massas acopladas, Three.js e live-canvas. |
| Ondas mecanicas | Onda em corda | Comprimento de onda, frequencia, periodo, velocidade, amplitude, probe na corda, perfil 1D e vetores transversais. | Fase 4 `ready` apos aprovacao manual; solucao senoidal viajante em `physics-core`, cena Three.js e live-canvas. |
| Ondas mecanicas | Onda longitudinal em mola | Mola diagonal, deslocamento material paralelo a propagacao, compressao/rarefacao, velocidade do pulso por rigidez/densidade linear ou `lambda f`, probe no elo e forca elastica local. | `analysis`; solucao senoidal longitudinal em `physics-core`, cena Three.js com espiras comprimindo/expandindo e live-canvas. |
| Ondas mecanicas | Superposicao e interferencia | Soma de ondas contrapropagantes, interferencia construtiva/destrutiva, componentes individuais, periodo, lambda/f acoplados e resultante no probe. | Fase 4 `ready` apos aprovacao manual; modelo analitico de duas ondas em Three.js; PixiJS fica opcional para mapa 2D futuro. |
| Ondas mecanicas | Ondas estacionarias | Nos, ventres, harmonicos, frequencia fundamental, periodo modal, lambda modal, velocidade por tensao/densidade linear e envelope. | Fase 4 `ready` apos aprovacao manual; solucao analitica de corda com extremidades fixas, controles de modo e frequencia derivada. |
| Som | Batimentos | Diferenca de frequencias, envoltoria, grafico de pressao, frequencia media e audio opcional. | Fase 4 `ready` apos aprovacao manual; modelo analitico 1D com campo volumetrico 3D de pontinhos de pressao no renderer Three.js, live-canvas e Web Audio API opcional posterior. |
| Som | Efeito Doppler | Fonte/observador em movimento, frequencia percebida, comprimentos de onda comprimido/alongado e limites classicos/subsonicos. | Fase 4 `ready` apos aprovacao manual; modelo Doppler 1D com fonte movel em loop, observador e frentes/pontinhos de pressao materializados como volume 3D em Three.js. |
| Optica | Reflexao e refracao | Lei de Snell, angulo critico, reflexao interna total, refletancia didatica, raios e pulsos. | Fase 4 `analysis`; modelo geometrico de interface plana, Three.js e live-canvas. |
| Optica | Lentes e espelhos | Foco, imagem real/virtual, aumento e equacao dos espelhos/lentes. | Fase 4 `analysis`; modelo paraxial de raios principais, banco optico Three.js e live-canvas. |
| Optica | Difracao e interferencia da luz | Fendas, padrao de intensidade, comprimento de onda, envoltoria, detector e distancia. | Fase 4 `analysis`; modelo escalar de Fraunhofer com tela Three.js, cor por wavelength e live-canvas. |
...
5. Concluido: expandir `Oscilacoes e Ondas > Ondas mecanicas` a partir de corda viajante, superposicao/interferencia e ondas estacionarias como `ready` apos aprovacao manual; adicionar `Onda longitudinal em mola` como `analysis` para cobrir propagacao longitudinal com compressao/rarefacao em uma mola diagonal, mantendo PixiJS como avaliacao opcional para densidade 2D.
6. Concluido: promover `Oscilacoes e Ondas > Som` com `Batimentos` e `Efeito Doppler` como `ready` apos aprovacao manual, mantendo audio audivel opcional e validando primeiro a leitura visual por campo volumetrico 3D de pontinhos de pressao.
7. Concluido: colocar `Oscilacoes e Ondas > Optica` em `analysis` com Reflexao/refracao, Lentes/espelhos e Difracao/interferencia.
8. Adicionar uma simulacao de `Termodinamica` baseada em diagramas de estado.
9. Adicionar uma simulacao de `Eletromagnetismo` baseada em campo eletrico ou circuito RC.
10. So depois avaliar bibliotecas especializadas por demanda real.
