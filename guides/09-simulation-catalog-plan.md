...
A primeira fatia visual de massa-mola foi antecipada em `Mecanica > Oscilacoes > Massa-mola vertical` para responder ao pedido feito diretamente nessa subarea da sidebar. Depois dela, `Oscilacoes e Ondas > Oscilacoes` passou a ter tres simulacoes em `analysis`: oscilador amortecido com regimes sub/critico/superamortecido, oscilador forcado com ressonancia e trabalho externo, e osciladores acoplados com modos normais e troca de energia. As demais ondas mecanicas, som e optica continuam planejadas, reaproveitando shell, blocos de graficos/tabela/formulas/teoria e padrao renderer-first.
