...
| Ondas mecanicas | Onda em corda | Comprimento de onda, frequencia, velocidade, amplitude, probe na corda, perfil 1D e vetores transversais. | Fase 4 `analysis`; solucao senoidal viajante em `physics-core`, cena Three.js e live-canvas. |
| Ondas mecanicas | Superposicao e interferencia | Soma de ondas contrapropagantes, interferencia construtiva/destrutiva, componentes individuais e resultante no probe. | Fase 4 `analysis`; modelo analitico de duas ondas em Three.js; PixiJS fica opcional para mapa 2D futuro. |
| Ondas mecanicas | Ondas estacionarias | Nos, ventres, harmonicos, frequencia fundamental, velocidade por tensao/densidade linear e envelope. | Fase 4 `analysis`; solucao analitica de corda com extremidades fixas, controles de modo e frequencia derivada. |
...
5. Em andamento: expandir `Oscilacoes e Ondas > Ondas mecanicas` a partir de corda viajante, superposicao/interferencia e ondas estacionarias em `analysis`, mantendo PixiJS como avaliacao opcional para densidade 2D.
6. Adicionar uma simulacao de `Termodinamica` baseada em diagramas de estado.
7. Adicionar uma simulacao de `Eletromagnetismo` baseada em campo eletrico ou circuito RC.
8. So depois avaliar bibliotecas especializadas por demanda real.
