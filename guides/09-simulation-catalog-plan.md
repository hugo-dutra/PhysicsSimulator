...
| Som | Batimentos | Diferenca de frequencias, envoltoria, grafico de pressao, frequencia media e audio opcional. | Fase 4 `analysis`; modelo analitico 1D com campo volumetrico 3D de pontinhos de pressao no renderer Three.js, live-canvas e Web Audio API opcional posterior. |
| Som | Efeito Doppler | Fonte/observador em movimento, frequencia percebida, comprimentos de onda comprimido/alongado e limites classicos/subsonicos. | Fase 4 `analysis`; modelo Doppler 1D com fonte movel, observador e frentes/pontinhos de pressao materializados como volume 3D em Three.js. |
...
6. Concluido: iniciar `Oscilacoes e Ondas > Som` com `Batimentos` e `Efeito Doppler` como `analysis`, mantendo audio audivel opcional e validando primeiro a leitura visual por campo volumetrico 3D de pontinhos de pressao.
7. Proxima decisao: seguir para `Oscilacoes e Ondas > Optica` ou revisar/aprovar manualmente as simulacoes de som.
