...
- Curvatura gravitacional em malha 3D: os mesmos parametros orbitais, massa central ajustavel de `0,05` a `20 M_terra`, intensidade da malha, alfa das linhas 3D, amplificacao didatica da massa orbital e um feixe neon desligado por padrao, ligavel, selecionavel por plano `XY`/`YZ`/`XZ`, movel em celulas U/V da grade e revelavel de `0%` a `100%` por um controle de trajetoria.
