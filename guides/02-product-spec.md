...
A sidebar deve ser organizada como arvore compacta: area principal expansivel, subarea expansivel e simulacoes como folhas marcadas por status. Subareas que contenham ao menos uma simulacao `analysis` iniciam abertas para destacar o que precisa de teste; subareas que contenham apenas simulacoes `ready` ou `planned` iniciam recolhidas. O fixture local de catalogo deve conter todas as simulacoes planejadas em `09-simulation-catalog-plan.md`. No estado atual, `Campo gravitacional e orbitas` e a segunda visualizacao, `Curvatura gravitacional em malha 3D`, permanecem `ready` depois das respectivas aprovacoes manuais. `Onda longitudinal em mola`, `Reflexao e refracao`, `Lentes e espelhos` e `Difracao e interferencia da luz` permanecem em `analysis`, aguardando teste manual. As demais simulacoes funcionais previamente aprovadas permanecem `ready`.
