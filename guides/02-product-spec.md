...
A sidebar deve ser organizada como arvore compacta: area principal expansivel, subarea expansivel e simulacoes como folhas marcadas por status. Subareas que contenham ao menos uma simulacao `analysis` iniciam abertas para destacar o que precisa de teste; subareas que contenham apenas simulacoes `ready` ou `planned` iniciam recolhidas. O fixture local de catalogo deve conter todas as simulacoes planejadas em `09-simulation-catalog-plan.md`. No estado atual, `Campo gravitacional e orbitas`, `Curvatura gravitacional em malha 3D` e `Onda longitudinal em mola` permanecem `ready` depois das respectivas aprovacoes manuais. `Reflexao e refracao`, `Lentes e espelhos` e `Difracao e interferencia da luz` permanecem em `analysis`, aguardando teste manual. As demais simulacoes funcionais previamente aprovadas permanecem `ready`.
...
- Card flutuante de parametros no canto inferior direito, fixo durante a rolagem, com sliders, inputs numericos, toggles, presets, play/pause, reset, velocidade de passagem do tempo e status da simulacao. O controle global de velocidade oferece atalhos unicos de `0,1x`, `0,25x`, `0,5x`, `0,75x` e `1x` em todas as simulacoes.
...
- Curvatura gravitacional em malha 3D: reutiliza o mesmo campo/orbita e a Lua didatica do sample, mas materializa uma grade volumetrica de cubos com tres pocos visuais; intensidade, nucleo de convergencia e alcance visual de cada deformacao crescem com a massa de referencia correspondente.
...
- Curvatura gravitacional em malha 3D: os mesmos parametros orbitais, massa central ajustavel de `0,05` a `20 M_terra`, intensidade da malha, alfa das linhas 3D com padrao `0,25`, densidade visual de cubos de `1x` a `10x` no total aproximado, amplificacao didatica da massa orbital, toggles independentes para corpo em orbita, rastro orbital e conjunto Lua/campo lunar, e um feixe neon desligado por padrao, ligavel, selecionavel por plano `XY`/`YZ`/`XZ`, movel em celulas U/V da grade e revelavel de `0%` a `100%` por um controle de trajetoria.
...
- Em `Curvatura gravitacional em malha 3D`, o espaco deve aparecer como cubos interligados por linhas originalmente paralelas e perpendiculares. Perto do astro central, do planeta e da Lua, varios vertices convergem exatamente para cada centro; fora dos nucleos, as linhas curvam de modo continuo, com intensidade e distancia perceptivel derivadas da massa de referencia. A Lua orbita o planeta com separacao visual minima suficiente para que o terceiro poco permaneça distinguivel e pode ser ligada/desligada junto de sua guia e deformacao, sem alterar a solucao orbital principal. A densidade inicia com `12^3` cubos e pode elevar aproximadamente em ate `10x` a quantidade total de cubos e vertices, aumentando de modo equilibrado as subdivisoes dos tres eixos sem alterar o volume ou a fisica. Cada aresta tambem pode receber amostras intermediarias para suavizar a curva. A cor deve formar um gradiente teal-amarelo-vermelho, ficando vermelha onde influencia visual e desvio geometrico sao maiores. Um feixe neon azul-claro nasce sobre uma linha escolhida da grade, fica praticamente reto longe dos nucleos e acumula mudancas transversais de direcao perto das massas; quando a influencia cai, segue reto pela tangente adquirida em vez de retornar a linha-base. Um slider revela continuamente de `0%` a `100%` desse percurso. A cena deve declarar que se trata de uma analogia visual amplificada, sem sugerir uma borda fisica finita do campo, uma dinamica real de tres corpos ou uma geodesica relativistica quantitativa.
- Nessa cena volumetrica, ocultar o corpo em orbita ou seu rastro remove somente esses elementos visuais. O poco orbital movel, a deformacao, os samples e a dinamica continuam ativos; o rastro dedicado tambem respeita o toggle geral `Trilha`.
...
- O usuario consegue abrir `Oscilacoes e Ondas > Ondas mecanicas > Onda longitudinal em mola` pela sidebar como simulacao pronta, aprovada manualmente, com mola diagonal, espiras comprimindo/expandindo paralelamente a propagacao, controles, graficos, tabela, formulas, teoria e warnings sincronizados.
