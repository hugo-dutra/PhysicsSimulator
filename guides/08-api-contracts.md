...
Para `Campo gravitacional e orbitas`, `specificGravitationalPotentialJoulesPerKilogram` e a grandeza fisica `-GM/r`. `spacetimeCentralDeformation` e `spacetimeOrbitingDeformation` sao escalas visuais adimensionais materializadas pelo motor para a analogia da malha; a segunda pode ser amplificada didaticamente e nenhuma delas participa da dinamica orbital. `fabricLineOpacity` pertence ao contrato visual do renderer, aceita `0..1`, usa `0.6` por padrao e nao participa da dinamica nem das series fisicas.
