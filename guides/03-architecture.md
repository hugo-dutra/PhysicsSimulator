...
- Em `Curvatura gravitacional em malha 3D`, o renderer cria uma unica `BufferGeometry` indexada com 12 divisoes por eixo, preservando 1.728 cubos e 6.084 arestas-base exclusivamente ortogonais. Cada aresta recebe um ponto intermediario: a geometria passa a 8.281 amostras e 12.168 segmentos renderizados para suavizar a curva sem aumentar a quantidade de cubos. O loop reutiliza os atributos dinamicos de posicao e cor e conserva uma copia imutavel da grade-base; a cada frame, aplica o mapeamento visual dos pocos do sample e um heatmap teal-amarelo-vermelho calculado pela influencia visual e pelo desvio da aresta, sem recriar geometria, material ou fisica. O feixe neon usa dois `InstancedMesh` de 72 segmentos (nucleo e glow), atualiza apenas matrizes reutilizadas e integra uma trajetoria propria iniciada sobre a linha-base: os mesmos pocos acrescentam curvatura transversal a direcao acumulada e, fora da influencia, os passos preservam a ultima tangente. `lightBeamProgressPercent` limita `InstancedMesh.count` e interpola o ultimo segmento para revelar continuamente `0..100%` do percurso. `spacetimeCentralInfluenceScale` e `spacetimeOrbitingInfluenceScale` transportam a escala de alcance derivada da massa; intensidade, nucleo em que varios vertices coincidem no centro e alcance suave crescem juntos, enquanto a dinamica orbital permanece inalterada.
