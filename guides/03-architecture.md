...
- Oscilacoes e Ondas: usar solucoes analiticas para osciladores simples e ondas senoidais; usar integradores ou malhas discretas para acoplamentos, amortecimento, forca externa e propagacao. A primeira fatia visual de oscilador adicional foi antecipada em `Mecanica > Oscilacoes > Massa-mola vertical`, com suporte superior fixo, mola helicoidal, massa esferica, eixo Z vertical em Three.js e samples compartilhados para deformacao da mola, vetores, graficos, tabela e formulas. A Fase 4 iniciou `Oscilacoes e Ondas > Oscilacoes` com oscilador amortecido analitico, oscilador forcado RK4 e osciladores acoplados RK4 em `analysis`, reutilizando o shell, a cena de molas e live-canvas. Avaliar PixiJS para ondas 2D densas e Web Audio API para som.
...
9. Expandir `Oscilacoes e Ondas` a partir dos tres osciladores em `analysis`, depois seguir para ondas mecanicas, som, optica, `Termodinamica` e `Eletromagnetismo` em fatias pequenas, sempre aplicando o gate de fidelidade antes de promover ou manter simulacoes como `analysis` ou `ready`.
