...
9. Expandir `Oscilacoes e Ondas` a partir dos tres osciladores e das tres primeiras ondas mecanicas aprovadas como `ready`, com `Batimentos` e `Efeito Doppler` aprovados como `ready` e a primeira fatia de Optica em `analysis`, depois seguir para `Termodinamica` e `Eletromagnetismo` em fatias pequenas, sempre aplicando o gate de fidelidade antes de promover ou manter simulacoes como `analysis` ou `ready`.
