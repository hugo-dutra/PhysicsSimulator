...
- `simulation-shell`: orquestra layout, playback, parametros, graficos, tabela e teoria. O controle global de playback compartilha os atalhos `0,1x`, `0,25x`, `0,5x`, `0,75x` e `1x` entre todas as simulacoes.
...
- Na cena de `Onda longitudinal em mola`, a linha helicoidal usa 1.536 vertices dinamicos, quatro vezes a resolucao original de 384. O buffer e criado uma vez e reutilizado pelo renderer; a maior amostragem suaviza as mesmas 54 voltas padrao sem alterar comprimento, compressao, samples ou o contrato fisico.
...
- Em `Campo gravitacional e orbitas`, a malha do espaco-tempo reutiliza um unico `PlaneGeometry` com grade `96 x 96` (9.409 vertices) e atualiza apenas o atributo de posicao. Um indice de `LineSegments` desenha linhas ortogonais alternadas sem as diagonais triangulares, mantendo curvas suaves e densidade visual controlada. O raio maximo da trajetoria de referencia define uma largura visual estavel para que a curvatura central seja percebida proxima da orbita, sem respirar em orbitas excentricas. Os dois pocos leem `spacetimeCentralDeformation`, `spacetimeOrbitingDeformation` e a posicao orbital do mesmo `KinematicsSample`; o renderer aplica apenas o mapeamento visual documentado, sem recalcular a orbita ou o campo. `fabricLineOpacity` atua somente no alfa do material wireframe, com padrao `0.6`.
- Em `Curvatura gravitacional em malha 3D`, o renderer cria uma unica `BufferGeometry` indexada com 12 divisoes por eixo no padrao, preservando 1.728 cubos e 6.084 arestas-base exclusivamente ortogonais. `latticeDensityMultiplier` aceita `1..10` e e convertido pela raiz cubica em divisoes iguais nos tres eixos; no maximo, 26 divisoes produzem 17.576 cubos e 19.683 vertices reais, um alvo aproximado de 10x sem multiplicar cada eixo por dez. Na densidade padrao cada aresta recebe um ponto intermediario, totalizando 8.281 amostras e 12.168 segmentos; acima de `1x`, os cubos menores assumem a suavizacao e esses intermediarios sao removidos para limitar o custo de linhas transparentes. O buffer e recriado somente no commit de uma mudanca de densidade; no loop, geometria, material e arrays tipados continuam reutilizados enquanto a CPU atualiza posicao, heatmap teal-amarelo-vermelho e alfa pelo mesmo mapeamento visual. `fabricLineOpacity` e o teto perto das massas, enquanto regioes distantes caem suavemente para 5,5% desse teto. O feixe neon usa dois `InstancedMesh` de 72 segmentos (nucleo e glow), preserva espessura e offsets aproximados quando a densidade muda, atualiza apenas matrizes reutilizadas e integra uma trajetoria propria iniciada sobre a linha-base: os mesmos pocos acrescentam curvatura transversal a direcao acumulada e, fora da influencia, os passos preservam a ultima tangente. `lightBeamProgressPercent` limita `InstancedMesh.count` e interpola o ultimo segmento para revelar continuamente `0..100%` do percurso. `spacetimeCentralInfluenceScale`, `spacetimeOrbitingInfluenceScale` e `spacetimeMoonInfluenceScale` transportam a escala de alcance derivada das massas de referencia; os campos equivalentes de deformacao materializam as tres intensidades no mesmo `KinematicsSample`. A Lua usa sua posicao secundaria, uma separacao minima de 1,55 celula-base e um ganho visual fixo declarado para formar um terceiro poco distinguivel; `moonEnabled` inclui ou exclui corpo, guia e poco somente no renderer, sem mudar a orbita principal de dois corpos. Intensidade, nucleo em que varios vertices coincidem no centro e alcance suave crescem juntos, enquanto densidade e demais escalas visuais nao alteram a dinamica orbital.
- `orbitingBodyVisible` atua apenas sobre a mesh do corpo orbital. `orbitTrailVisible` atua sobre a elipse de referencia e condiciona o buffer de rastro progressivo junto com o overlay geral `Trilha`; nenhum dos dois interrompe o poco orbital ou o calculo dos samples. `moonEnabled` atua sobre a Lua, sua guia local e a inclusao do terceiro poco no mapeamento volumetrico e no feixe, mas preserva as coordenadas secundarias materializadas no sample e toda a dinamica orbital principal.
...
- As primeiras simulacoes de `Mecanica > Cinematica` reutilizam um motor analitico comum e uma cena Three.js 2.5D compartilhada para MRU, MUV/queda livre, lancamento obliquo e MCU. O mesmo modulo analitico tambem atende `Massa-mola vertical`, `Maquina de Atwood`, `Forca centripeta em curva`, `Trabalho e energia em trilho`, `Colisoes 1D e 2D`, `Equilibrio de particula`, `Torque, alavancas e centro de massa`, `Rotacao de corpo rigido`, `Rolamento sem escorregamento`, `Campo gravitacional e orbitas`, `Curvatura gravitacional em malha 3D`, `Hidrostatica e empuxo` e `Continuidade e Bernoulli`, com campos extras de forca, tensao, atrito, trabalho, dissipacao, percentual de perda, momento linear, impulso, torque, centro de massa, bracos de alavanca, momento de inercia, grandezas angulares, campo gravitacional, escalas de deformacao/alcance, pressao, vazao, empuxo, massa, densidade derivada, fracao submersa, deformacao estatica da mola e energia elastica/gravitacional quando o modelo exige. O renderer compartilhado pode especializar a cena quando o fenomeno exigir uma leitura didatica propria, como a mola vertical, a rampa em U de energia, a gangorra de torque/alavancas, a roda de rolamento, o tecido orbital 2D, a malha cubica volumetrica ou o tanque transparente de hidrostatica, desde que a especializacao continue usando `KinematicsSample` como unica fonte para corpo, rastro, vetores e paineis. Cada simulacao ainda declara seus proprios parametros, formulas, teoria, limites e graficos.
...
- O catalogo local deve incluir todas as simulacoes planejadas em `09-simulation-catalog-plan.md`, agrupadas por `topicPath`. `reflection-refraction`, `lenses-mirrors` e `light-diffraction-interference` ficam como `analysis` ate teste manual; `longitudinal-wave`, `gravitational-space-lattice`, `gravitational-field-orbits` e as demais simulacoes funcionais previamente aprovadas ficam como `ready`, e os demais itens permanecem `planned` ate terem motor, cena, graficos, tabela, formulas e teoria. Uma simulacao so deve virar `ready` depois da validacao manual do dono do projeto.
...
9. Expandir `Oscilacoes e Ondas` a partir dos tres osciladores e das quatro ondas mecanicas aprovadas como `ready`, manter `Batimentos` e `Efeito Doppler` aprovados como `ready` e a primeira fatia de Optica em `analysis`, depois seguir para `Termodinamica` e `Eletromagnetismo` em fatias pequenas, sempre aplicando o gate de fidelidade antes de promover ou manter simulacoes como `analysis` ou `ready`.
