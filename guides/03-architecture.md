...
- Oscilacoes e Ondas: usar solucoes analiticas para osciladores simples e ondas senoidais; usar integradores ou malhas discretas para acoplamentos, amortecimento, forca externa e propagacao. A primeira fatia visual de oscilador adicional foi antecipada em `Mecanica > Oscilacoes > Massa-mola vertical`, com suporte superior fixo, mola helicoidal, massa esferica, eixo Z vertical em Three.js e samples compartilhados para deformacao da mola, vetores, graficos, tabela e formulas. A area `Oscilacoes e Ondas` permanece como espaco dos proximos osciladores acoplados, ondas, som e optica. Avaliar PixiJS para ondas 2D densas e Web Audio API para som.
...
- O catalogo local deve incluir todas as simulacoes planejadas em `09-simulation-catalog-plan.md`, agrupadas por `topicPath`. Depois da validacao manual, `simple-pendulum`, `mass-spring`, `uniform-linear-motion`, `uniformly-accelerated-motion`, `projectile-motion`, `inclined-plane-friction`, `atwood-machine`, `centripetal-force-curve`, `work-energy-track` e `collisions-1d-2d` ficam como `ready`; `uniform-circular-motion`, `particle-equilibrium`, `torque-levers-center-mass`, `rigid-body-rotation`, `rolling-without-slipping`, `gravitational-field-orbits`, `hydrostatics-buoyancy` e `continuity-bernoulli` ficam como `analysis` ate teste manual; demais itens permanecem `planned` ate terem motor, cena, graficos, tabela, formulas e teoria. Uma simulacao so deve virar `ready` depois da validacao manual do dono do projeto.
...
8. Adicionar rolamento, gravitacao e fluidos basicos para fechar a cobertura mecanica introdutoria funcional em `analysis`.
9. Expandir por `Oscilacoes e Ondas`, `Termodinamica` e `Eletromagnetismo` em fatias pequenas depois da antecipacao de `Mecanica > Oscilacoes > Massa-mola vertical`, sempre aplicando o gate de fidelidade antes de promover ou manter simulacoes como `analysis` ou `ready`.
