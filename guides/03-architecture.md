...
9. Expandir `Oscilacoes e Ondas` a partir dos tres osciladores e das tres primeiras ondas mecanicas aprovadas como `ready`, manter `Batimentos` e `Efeito Doppler` como proximas simulacoes de som em `analysis`, depois seguir para optica, `Termodinamica` e `Eletromagnetismo` em fatias pequenas, sempre aplicando o gate de fidelidade antes de promover ou manter simulacoes como `analysis` ou `ready`.
