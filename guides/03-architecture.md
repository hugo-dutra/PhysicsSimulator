...
- O catalogo local deve incluir todas as simulacoes planejadas em `09-simulation-catalog-plan.md`, agrupadas por `topicPath`. Depois da validacao manual, `simple-pendulum`, `uniform-linear-motion`, `uniformly-accelerated-motion`, `projectile-motion`, `inclined-plane-friction`, `atwood-machine` e `centripetal-force-curve` ficam como `ready`; `uniform-circular-motion`, `work-energy-track`, `collisions-1d-2d`, `particle-equilibrium`, `torque-levers-center-mass` e `rigid-body-rotation` ficam como `analysis` ate teste manual; demais itens permanecem `planned` ate terem motor, cena, graficos, tabela, formulas e teoria. Uma simulacao so deve virar `ready` depois da validacao manual do dono do projeto.
